...
class DatabaseManager:
    ...
    def get_expired_prizes(self):
        """Получить призы, которые можно отправить повторно"""
        conn = sqlite3.connect(self.database)
        with conn:
            cur = conn.cursor()
            # Находим призы, которые были отправлены, но у них меньше 3 победителей
            cur.execute('''
                SELECT p.prize_id, p.image, 
                       COUNT(w.prize_id) as winners_count
                FROM prizes p
                LEFT JOIN winners w ON p.prize_id = w.prize_id
                WHERE p.used = 1
                GROUP BY p.prize_id
                HAVING winners_count < 3
                ORDER BY p.prize_id
            ''')
            return cur.fetchall()
    
    def reset_prize_for_retry(self, prize_id):
        """Сбросить приз для повторной отправки"""
        conn = sqlite3.connect(self.database)
        with conn:
            # Сбрасываем флаг used
            conn.execute('''UPDATE prizes SET used = 0 WHERE prize_id = ?''', (prize_id,))
            # Удаляем старых победителей этого приза (по желанию)
            # conn.execute('''DELETE FROM winners WHERE prize_id = ?''', (prize_id,))
            conn.commit()
    
    def can_get_prize_retry(self, prize_id, user_id):
        """Проверить, может ли пользователь получить приз повторно"""
        conn = sqlite3.connect(self.database)
        with conn:
            cur = conn.cursor()
            # Проверяем, получал ли уже пользователь этот приз
            cur.execute('SELECT * FROM winners WHERE user_id = ? AND prize_id = ?', (user_id, prize_id))
            if cur.fetchone():
                return False
            return True
    
    def add_prize_retry(self, user_id, prize_id):
        """Добавить запись о получении приза при повторной отправке"""
        return self.add_winner(user_id, prize_id)  # Используем существующий метод
